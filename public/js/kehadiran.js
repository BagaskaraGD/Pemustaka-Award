document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderActivities(activitiesToRender) {
        ...
        if (paginatedActivities.length === 0 && activitiesToRender.length > 0) {
            ...
            // Handled by other displayMessage calls
        } else {
            paginatedActivities.forEach((activity) => {
                ...
                header.className = `p-4 flex items-center justify-between cursor-pointer bg-white hover:bg-gray-50 transition-colors duration-200 overflow-x-auto`;
                ...
                <div class="flex items-center flex-shrink-0 mr-4">
                    ...
                    <div>
                        <h3 class="font-bold text-lg text-gray-800 whitespace-nowrap">${activity.judul_kegiatan}</h3>
                        <p class="text-sm text-gray-600 whitespace-nowrap">${activity.lokasi}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-3 flex-shrink-0">
                    <span class="text-sm font-semibold text-gray-500 whitespace-nowrap">${activity.sesi.length} Sesi</span>
                    <span class="inline-flex items-center px-3 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-semibold whitespace-nowrap">
                        ${activityPoin} Poin Total
                    </span>
                    <button class="bg-green-500 hover:bg-green-600 text-white text-xs font-semibold py-1.5 px-3 rounded-md shadow-sm transition-all duration-200 ease-in-out transform hover:scale-105 hover:shadow-lg print-activity-certificate-button whitespace-nowrap" data-kegiatan-id="${activity.id_kegiatan}">
            ...
            `;
                ...
                // Kode untuk sessionDetails dan sessionTable tetap sama seperti perbaikan sebelumnya
                const sessionDetails = document.createElement("div");
                sessionDetails.className = `overflow-hidden max-h-0 transition-all duration-500 ease-in-out bg-gray-50`;
                ...
                tableWrapper.className = "w-full overflow-x-auto p-4";

                const sessionTable = document.createElement("table");
                sessionTable.className = `w-full text-gray-800 min-w-[550px]`;
            ...
            `;
                ...
                activity.sesi.forEach((sesi, sesiIndex) => {
                    ...
                    const jamKegiatanFormatted = sesi.jam_kegiatan;
                    sessionRow.innerHTML = `
                    <td class="p-3 text-left text-sm text-gray-700 pl-10 border-l-4 border-blue-200">Sesi ${
                        sesiIndex + 1
                    }</td>
                    <td class="p-3 text-left text-sm text-gray-600"><div class="flex items-center space-x-2"><i class="far fa-calendar-alt text-blue-500"></i><span>${tglKegiatanFormatted}</span></div></td>
                    <td class="p-3 text-left text-sm text-gray-600"><span class="inline-flex items-center px-2 py-1 rounded-md bg-purple-100 text-purple-800 text-xs font-semibold"><i class="far fa-clock text-purple-500 mr-1"></i> ${jamKegiatanFormatted}</span></td>
                    <td class="p-3 text-left text-sm text-gray-600">${
                        sesi.nama_pemateri
                    }</td>
                    <td class="p-3 text-center"><span class="inline-flex items-center px-3 py-1 rounded-full bg-green-100 text-green-800 text-xs font-semibold">${
                        sesi.bobot
                    } Poin</span></td>
                `;
                ...
                tableWrapper.appendChild(sessionTable);
                sessionDetails.appendChild(tableWrapper);
            ...
            .forEach((header) => {
                header.addEventListener("click", (event) => {
                    ...
                    if (content.classList.contains("max-h-0")) {
                        ...
                        content.classList.add("max-h-[1000px]"); // Use a large max-height

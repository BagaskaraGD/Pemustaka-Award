document.addEventListener("DOMContentLoaded", function () {
    ...
    const apiUrlGet = `http://127.0.0.1:8000/api/pinjaman-count/${idCivitas}`;
    const apiUrlPoin = "http://127.0.0.1:8000/api/range-kunjungan/pinjaman";

    fetch(apiUrlGet)
        .then((response) => response.json())
        .then((data) => {
            const count = Number(data.count);
            if (isNaN(count)) throw new Error("Jumlah Pinjaman tidak valid");

            console.log("Jumlah Pinjaman:", count); // DEBUG
            ...
            return fetch(apiUrlPoin)
                ...
                .then((bobotData) => {
                    ...
                    const apiUrlPut = `http://127.0.0.1:8000/api/rekap-poin/pinjaman/${idCivitas}/${count}/${totalPoin}`;

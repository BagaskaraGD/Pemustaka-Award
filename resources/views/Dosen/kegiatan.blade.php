...
@section('content')
    <h2
        class="text-3xl md:text-4xl font-extrabold font-rubik mb-6 text-gray-800 border-b-2 border-red-600 pb-2 inline-block">
        Kegiatan
    </h2>
    ...
    <div class="flex flex-col sm:flex-row items-stretch mb-8 gap-4">
        ...
        <form action="{{ route('kehadiran.store') }}" method="POST"
            ...
            <input type="text" name="kode" placeholder="Kode Kegiatan" {{-- Ganti placeholder agar lebih ringkas --}}
                class="flex-grow border border-gray-300 p-2 rounded-lg shadow-inner focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm font-rubik placeholder-gray-400 transition-all duration-200 ease-in-out focus:placeholder-opacity-0"
                {{-- Kurangi p-2.5 jadi p-2 dan text-base jadi text-sm --}} id="koderandom">
            <button type="submit"
                class="w-full sm:w-auto bg-[#880e4f] text-white text-sm font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-red-700 transition-all duration-200 ease-in-out flex items-center justify-center space-x-2 transform hover:scale-105 hover:shadow-lg">
    ...
    <div class="flex justify-end mt-6 font-rubik">
        <div class="bg-white border border-gray-200 px-6 py-3 rounded-xl shadow-md flex items-center space-x-3">
            ...
            <div class="text-3xl font-extrabold text-[#880e4f]" id="totalpoin">0</div>
        </div>
    </div>
